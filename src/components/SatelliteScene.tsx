...
class WebGLBoundary extends Component<{ children: ReactNode; fallback: ReactNode }, { hasError: boolean }> {
  ...
  componentDidCatch(err: any) {
    console.warn("WebGL Context error in SatelliteScene, using 2D fallback:", err);
  }
  render() {
...
function AdaptivePerf() {
  const { gl } = useThree();
  const fps = useRef({ frames: 0, last: performance.now() });
  useFrame(() => {
    fps.current.frames++;
...
const OrbFallback2D = () => (
  <div className="w-full h-full flex items-center justify-center relative">
    <div className="w-64 h-64 rounded-full bg-gradient-to-tr from-cosmic-purple via-cosmic-teal to-blue-500 blur-xl opacity-60 animate-pulse" />
    <div className="absolute w-56 h-56 rounded-full border border-cosmic-teal/50 bg-[#061830] flex items-center justify-center shadow-2xl">
      <span className="text-cosmic-teal font-display font-bold text-lg tracking-wider">SAMYAM 3D</span>
    </div>
  </div>
);
